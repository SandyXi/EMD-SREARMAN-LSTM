...
if args.fn == "":
    ...
    exit(0)
if args.host == "":
    print("Please input HDFS host")
    exit(0)
if args.p == "":
    print("Please input HDFS port")
    exit(0)
clientPath = args.host + ":" + args.p
client = Client(clientPath)

#单独提取出客流量那一列
filePath = args.fp
filepushpath1 = filePath + "/input/" + args.fn
with client.read(filepushpath1, encoding='utf-8') as push:
    flowdata = push.read(read_csv(filepushpath1, header=None, usecols=[1]))

filepoppath1 = filePath + 'imf1.csv'
with client.write(filepoppath1, encoding='utf-8') as pop:
    pop.write(flowdata.to_csv(filepoppath1, index=None, columns=None, header=None))

plt.figure('原数据')
plt.plot(flowdata)
#载入到时间序列数据

filepushpath2 = filePath + 'imf1.csv'
data = push.read(read_csv(filepushpath2,header=None))
...
filepoppath2 = filePath + 'imf2.csv'
pop.write(dataframe.to_csv(filepoppath2,index=None,columns=None,header=None))

# lstm


# load the dataset导入数据：
dataframe = push.read(read_csv(filepushpath1,usecols=[1],engine='python'))
filepushpath3 = filePath + 'imf2.csv'
dataframe_1 = push.read(read_csv(filepushpath3,engine='python'))
...
filepoppath3 = '/forecast/predict_data.csv'
pop.write(output.to_csv(filepoppath3,index=None,columns=None,header=None))
...
filepoppath4 = filePath + '/output/result.png'
pop.write(plt.savefig(filepoppath4))
